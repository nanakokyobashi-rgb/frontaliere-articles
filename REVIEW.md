...
- **Un 🔴 NUOVO ancorato solo a righe che nessuno ha toccato dall'ultima review viene classificato come candidato `DECLASSIFIED-UNCHANGED-LINE`, ma NON viene approvato**. Il classificatore conserva il risparmio di token e lo espone come `UNCHANGED-LINE-RECHECK`: il gate resta rosso e registra una sola recheck deterministica (`REVIEW_GATE_UNCHANGED_LINE_RECHECK`), senza avviare un ciclo N di fixer. Solo una review nuova che rimuove o conferma il finding puo' chiudere il gate. Se sei davvero convinto che sia una regressione, dichiaralo `🔴 Important: [regression]` e resta bloccante. La classificazione e' conservativa: non scatta senza anchor di riga, non scatta se il delta non e' calcolabile, non scatta se anche un solo path citato non e' stato confrontato, e non scatta mai su un finding che il parser non sa delimitare.
