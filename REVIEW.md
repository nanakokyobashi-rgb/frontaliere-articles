...
- **Un 🔴 NUOVO ancorato solo a righe che nessuno ha toccato dall'ultima review viene classificato** (`DECLASSIFIED-UNCHANGED-LINE` nel log), ma non e' un verdetto approvante: il review gate resta fail-closed finche' una review successiva non lo risolve esplicitamente. Quel codice e' esattamente quello gia' giudicato: se il rilievo valeva anche allora, ripetilo con lo stesso id stabile e una review pulita; se e' una regressione, dichiaralo `🔴 Important: [regression]`, che resta bloccante. La classificazione e' conservativa: non scatta senza anchor di riga, non scatta se il delta non e' calcolabile, non scatta se anche un solo path citato non e' stato confrontato, e non scatta mai su un finding che il parser non sa delimitare.
...
7. **Claim di performance non validato** → PR che dichiara uno speedup senza una misura comparabile before/after, con baseline pre-merge, su tier high → 🔴: "claim non validato; allega la misura prima del merge oppure rimuovi il claim e l'ottimizzazione". Dichiarare `revert-risk` non è una misura e non soddisfa questo requisito.
8. **Fix provabile solo da una run su `main`** (#151) → la PR tocca `.github/workflows/**` o la config dell'action Claude (`claude_args`, `settings`, sandbox, `permissionMode`) per un bug osservabile solo a runtime (sandbox/bwrap, permessi, rate-limit, dispatch) — né il diff né i test possono dimostrare che il fix funziona, solo una run reale può farlo. Se in questo caso il body chiude l'issue con `Closes #<n>` invece di lasciarla aperta con `Refs #<n>` + label `awaiting-production-proof` → 🔴 process: "questa classe di fix non è verificabile dal diff — chiudere l'issue al merge dichiara risolto ciò che nessuno ha ancora misurato in produzione". Non si applica alle PR `.github/workflows/**` la cui correttezza SI dimostra dal diff/test (es. una regex, un `if:`, un nome di job) — solo a quelle il cui unico criterio di verità è l'esecuzione.
