...
| **minimal (body-only)** | Body corretto sulla STESSA HEAD dopo un verdetto non approvante (`scripts/ci/body-rereview-admission.mjs`) | Il codice e' identico per costruzione: si rigiudica SOLO il body. Il bundle porta `## Code contribution unchanged` e ogni 🔴 di codice aperto va riportato identico, mai chiuso con `Fix di`. Un `## LGTM` pulito sulla HEAD resta sticky (nessuna review comprata da un body edit) e i verdetti sulla stessa HEAD sono limitati a 3. |
