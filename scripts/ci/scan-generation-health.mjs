    ...
    evaluate(m) {
      ...
      return {
        ...
        body: [
          ...
          'Il file è `adapted` nel `loop-sync-manifest.json` dal 2026-09-04 (issue #806), e nessun mirror porta',
          '`generator/`: le due copie sono divergenti di proposito, quindi la fix va fatta QUI per prima — è qui che',
          'la generazione gira — e poi RIAPPLICATA A MANO sul sito, mai copiata sopra la sua, perché la copia del',
          'sito non ha il breaker host-unreachable (#475, #767).',
    ...
    evaluate(m) {
      ...
      return {
        ...
        body: [
          ...
          '',
          'La catena si costruisce in `generator/scripts/lib/ai-models.mjs`, che dal 2026-09-04 è `adapted`',
          'nel `loop-sync-manifest.json` (issue #806) e non sta sotto nessun prefisso mirrorato: le due copie',
          'sono divergenti, quindi una fix va applicata QUI e poi riapplicata a mano sul sito, mai copiata',
          'sopra la sua — la copia del sito non ha il breaker host-unreachable. Questa condizione, invece, OSSERVA e basta: non tocca il',
