...
/**
 * ── ADATTAMENTO DICHIARATO (manifest: `mergePreviewCheck`) ────────────────
 *
 * Sul sito l'import di `duplicateDeclarations.mjs` è statico. Quel modulo
 * importa `typescript` (parsa l'AST vero invece di fare regex sul testo), e
 * questo repo NON ha typescript fra le sue dipendenze.
 *
 * Con l'import statico l'intera catena si rompe a monte: `auto-merge-eval.mjs`
 * non si CARICA nemmeno, quindi l'auto-merge fallisce su ogni PR — non per un
 * gate non soddisfatto, ma per un `ERR_MODULE_NOT_FOUND` prima di qualunque
 * valutazione.
 *
 * La via ovvia — aggiungere `npm ci` al workflow di auto-merge — costerebbe
 * l'installazione di playwright, sharp e @huggingface/transformers a ogni
 * merge, per un gate che qui vale poco: intercetta due PR che aggiungono la
 * stessa dichiarazione top-level, cioè un `SyntaxError` di esbuild al merge, e
 * questo repo non bundla con esbuild (builda con `tsx`).
 *
 * Quindi il modulo si carica solo se typescript è risolvibile, e altrimenti il
 * check si dichiara saltato — usando lo stesso pattern di "skip conservativo"
 * che questo file già applica a rete assente, sha irraggiungibile e merge-tree
 * non risolvibile. Il giorno in cui typescript entrasse fra le dipendenze, il
 * gate si riattiva da solo senza toccare niente.
 */
let findDuplicateTopLevelNames = null;
let tsUnavailableReason = '';
try {
  createRequire(import.meta.url).resolve('typescript');
  ({ findDuplicateTopLevelNames } = await import('./duplicateDeclarations.mjs'));
} catch (e) {
  // Il messaggio di ERR_MODULE_NOT_FOUND è multilinea (include il require
  // stack): su una riga sola resta leggibile in un log di Actions.
  tsUnavailableReason = String(e && e.message ? e.message : e)
    .replace(/\s+/g, ' ')
    .slice(0, 90);
}
...
export function checkMergePreviewDuplicates({ headSha, baseSha, mergeBaseSha, cwd = process.cwd() }) {
  if (!findDuplicateTopLevelNames) {
    return {
      ok: true,
      reason: `merge-preview: typescript non risolvibile in questo repo (${tsUnavailableReason}) — check saltato, non blocca. Vedi la nota in cima al file.`,
    };
  }
