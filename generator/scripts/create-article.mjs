...
async function translateArticle(data) {
  async function callWithRetry(prompt, maxTokens, label) {
    ...
    // Un tentativo: chiama callLLM e prova il parse, senza mai lanciare per un
    // errore di callLLM — lo riporta come `infra: true` cosi' la cascata qui
    // sotto lo tratta come il fallimento di QUESTO tentativo, non come
    // un'eccezione che salta i tentativi rimasti. Stessa causa di #391
    // (requestHeadlineSelection): senza questo, un rate limit o una cascata
    // esaurita su UNA delle tre chiamate risalivano non taggati oltre
    // generateAndValidateArticle (che non avvolge translateArticle in un
    // try/catch) e facevano fallire l'intero run al posto di consumare un
    // tentativo della cascata di retry qui sotto.
