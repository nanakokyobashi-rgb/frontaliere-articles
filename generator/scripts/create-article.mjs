...
// per id, e il modello ha risposto `3` a un pool di 2 headline motivando un
// articolo del corpus (run 31402084443). Le due liste NON devono tornare a
 ...
 *
 * Il ramo esatto guarda i ledger di TUTTE le sezioni (issue #251): dopo #246 il
 * classificatore frontaliere accetta anche i dati aggregati Italia-Svizzera, che
 * passano pure il classificatore nazionale, quindi lo STESSO documento di fonte
 * poteva generare due articoli pubblicati come contenuti distinti — 5 casi gia'
 * presenti nel corpus il 2026-08-13. La chiave e' l'URL normalizzato, cioe'
 * l'identita' della FONTE: gli slug dei due articoli sono diversi per
 * costruzione e confrontarli non avrebbe trovato niente. Il blocco e' quindi
 * sul contenuto duplicato, non sulla copertura dello stesso tema — due pezzi
 * sulla stessa vicenda da due URL diversi restano ammessi in entrambe le
 * sezioni. Vedi lib/cross-section-dedup.mjs.
 */
...
- Fisco: 3 livelli (IFD federale + cantonale + comunale); ogni cantone ha legge/moltiplicatore propri (comuni: moltiplicatore sul cantonale).
- Competenze (MAI confonderle): AFC/ESTV = IFD + IVA; amministrazioni CANTONALI = cantonale/comunale; UFAS/BSV = previdenza sociale (AVS/AI/LPP), NON fissa imposte; UST/BFS = statistiche, NON fissa aliquote.
- Contributi su salario: AVS/AI/IPG 5.3% dipendente (10.6% tot. con datore), AD/AC 1.1% fino a massimale annuo, LAINF/LAA 0.7–1.5% secondo settore; LPP (salario coordinato): 7% (25–34), 10% (35–44), 15% (45–54), 18% (55+ fino a eta' di riferimento).
- LAMal: assicurazione malattia OBBLIGATORIA per residenti, da stipulare entro 3 mesi dall'arrivo; privata a premi pro capite, NON tassa ne' contributo salariale. Franchigie adulti CHF 300/500/1000/1500/2000/2500. Premi per cantone/regione; riduzione premi (sussidio) cantonale.
- Permessi stranieri (SEM): L breve durata (fino a 1 anno), B dimora (rinnovabile), C domicilio (di norma 10 anni residenza, 5 per UE/AELS), G frontaliere. Naturalizzazione ordinaria: permesso C + 10 anni residenza (8–18 anni contano doppio) + requisiti cantonali/comunali.
- Locazione (CO art. 253 segg., diritto FEDERALE uguale in ogni cantone): deposito cauzionale max 3 mesi pigione, conto vincolato intestato all'inquilino; disdetta del locatore valida solo su modulo ufficiale cantonale; contestazione entro 30 giorni all'autorita' di conciliazione.
- Lavoro: NESSUN salario minimo federale (alcuni cantoni ne hanno uno proprio). Orario max 45 ore/sett (industria, uffici, vendita) o 50 ore altri settori. Vacanze min 4 sett (5 sotto i 20 anni). Maternita' 14 sett all'80% (IPG); paternita' 2 sett. Disdetta: 1/2/3 mesi (1° anno / 2°–9° / dal 10°).
- Democrazia diretta: 4 date di voto federale/anno. Iniziativa popolare 100'000 firme in 18 mesi; referendum facoltativo 50'000 firme in 100 giorni. Modifica costituzionale: DOPPIA maggioranza (popolo E cantoni).
...
- Cifre indicizzate ogni anno (massimale 3° pilastro 3a, soglia/deduzione coordinamento LPP, massimale AD/AC, premi medi LAMal, salario mediano): NON dare importi precisi se incerto sull'anno; spiega il meccanismo, rimanda alla fonte ufficiale.`;
...
async function llmFactCheck(contentIt, sourceContent = '', sourceUrl = '') {
  ...
  const isEvergreen = !sourceContent || sourceContent.length < 100 || sourceUrl.startsWith('evergreen://') || sourceUrl.startsWith('stats-bfs://') || sourceUrl.startsWith('stats-astra://');
...
function formatStatsAstraPrompt(cadence, requestedPeriod, section, data) {
  ...
  const targetPeriod = requestedPeriod || (isWeekly ? weekly?.period : monthly?.period) || 'n/d';
  const minWords = 550;
  ...
  if (isWeekly) {
    ...
    const history = Array.isArray(data?.weekly?.history) ? data.weekly.history.slice(-12) : [];
  ...
  const monthlyHistory = Array.isArray(data?.monthly?.history) ? data.monthly.history.slice(-12) : [];
...
// (run 31402084443). Qui si ritenta con un promemoria esplicito, e se il
// modello non si corregge la selezione fallisce in modo pulito.
//
...
- Se la fonte NON specifica un dato: omettilo; niente placeholder (es. "non specificato").
...
DATI ASSENTI NELLA FONTE: omettili; niente placeholder (es. "non specificato") e niente dettagli inventati.
    ...
    for (const field of [
      ...
      for (const faqIndex of faqIndexes) {
        ...
        const readField = (content) => faqPart
          ...
          : content?.[field];
        const writeField = (content, value) => {
          if (!content || typeof content !== 'object') return;
          if (!faqPart) {
            content[field] = value;
            return;
          }
          if (!Array.isArray(content.faq)) content.faq = [];
          while (content.faq.length <= faqIndex) content.faq.push({});
          if (!content.faq[faqIndex] || typeof content.faq[faqIndex] !== 'object') content.faq[faqIndex] = {};
          content.faq[faqIndex][faqPart] = value;
        };
        const readRetryValue = (parsed) => faqPart
          ? parsed?.faq?.[0]?.[faqPart] ?? parsed?.faq?.[faqIndex]?.[faqPart] ?? parsed?.[field]
          : parsed?.[field];
        const retryFieldLabel = faqPart ? `faq[${faqIndex}].${faqPart}` : field;
      ...
      const valoreTradotto = readField(data.content[locale]);
      ...
      writeField(data.content[locale], itValue);
      }
    }
  }
