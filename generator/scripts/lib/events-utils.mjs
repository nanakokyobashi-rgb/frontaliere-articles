 ...
 * requests, and a crawler's own visit-loop time budget does not cover it —
 * that cap stops the VISIT loop and nothing after it (site incident: this gap
 * cancelled `crawl-events` daily from 2026-07-07 on, freezing the myswitzerland
 * checkpoint because the run was killed before the slice/checkpoint write).
 */
