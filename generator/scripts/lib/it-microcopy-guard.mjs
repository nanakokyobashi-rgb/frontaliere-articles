 ...
 * occhio umano. Il 2026-08-09 sono usciti in produzione:
 *
 ...
 *
 * `normalizeTitleCasing()` in create-article.mjs conosce gia' i toponimi
 * (TITLE_CASING_PROPER_NOUNS contiene 'ticino'), ma ESCE SUBITO quando il
 * titolo non e' ne' Title Case ne' TUTTO MAIUSCOLO:
 *
 *     if (!looksTitleCase && !isShouting) return s;
 *
 * Un titolo gia' in sentence case con un toponimo minuscolo cade esattamente
 * in quel ramo: la tabella dei nomi propri c'era, il codice che la consulta
 * non veniva mai raggiunto. Questa guardia e' INCONDIZIONATA per costruzione —
 * non ha un ramo di uscita anticipata.
 *
 * ## Cosa NON e'
 *
 * NON e' un correttore grammaticale. Ogni regola qui dentro e' una funzione
 * totale su un insieme CHIUSO di token, scelta perche' la forma sbagliata non
 * ha nessuna lettura corretta in italiano. Tutto cio' che richiede di capire
 * la frase resta fuori, e resta fuori DI PROPOSITO: vedi «Fuori copertura».
 *
 * Le quattro regole, nell'ordine in cui vanno applicate (l'ordine conta: R3
 * puo' produrre un plurale che R1 deve poi riarticolare — «i stipendio» → R3 →
 * «i stipendi» → R1 → «gli stipendi»):
 *
 ...
 *
 * @see generator/tests/it-microcopy-guard.test.mjs
 */
 ...
 * (Ticino resta Ticino anche in un titolo EN/DE/FR).
 */
 ...
 *
 * `svizzera` da sola vale 136 campi IT: escluderla non e' una rifinitura, e'
 * la decisione che rende la regola applicabile.
 */
export const TOPONYMS_EXCLUDED_HOMOGRAPHS = Object.freeze({
  svizzera: 'aggettivo di uso comune — «economia svizzera», «busta paga svizzera»: 136 campi IT misurati, tutti corretti in minuscolo',
 ...
 * caso vivo (14 campi pubblicati), gli altri sono i suoi vicini nello stesso
 * dominio.
 *
 ...
 * generale…», «Tutto quello che devi sapere…» — 5 excerpt pubblicati) sono
 * fuori di proposito: toglierle cambierebbe il testo, non lo ripulirebbe.
 */
