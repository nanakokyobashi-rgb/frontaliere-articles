...
test('il gate di inattività usa il massimo delle date, non l\'ultimo elemento', () => {
  assert.match(WORKFLOW, /--json commits,headRepository,headRepositoryOwner,headRefName,headRefOid/);
...
test('il fallback DELETE confronta REF_SHA e HEADSHA prima di cancellare', () => {
  ...
  assert.match(beforeDelete, /gh api --include/);
  assert.match(beforeDelete, /REF_SHA=.*probe_body/);
