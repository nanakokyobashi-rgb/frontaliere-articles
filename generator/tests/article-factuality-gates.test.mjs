/**
 * PORTATO da valerielinc-ops/frontaliere-si-o-no `tests/article-factuality-gates.test.ts`
 * (manifest: `adapted`). La libreria sotto test — generator/scripts/lib/
 * article-factuality-gates.mjs — è arrivata qui col trasporto del generatore,
 * ma la sua suite era rimasta SOLO sul sito: è esattamente il punto cieco che
 * il loop-drift-check non vede (l'assenza di un test da un lato). Questi gate
 * girano a ogni generazione di articolo IN QUESTO repo, quindi la loro
 * regressione va fermata qui, non là.
 *
 * ## ADATTAMENTI rispetto al sito
 *  - `node:test` + generator/tests/lib/expect-shim.mjs al posto di vitest:
 *    questo repo non ha node_modules né npm ci in CI (vedi tests.yml). I corpi
 *    dei test restano quasi byte-identici all'originale, apposta: un futuro
 *    riallineamento con la suite del sito è copia + questa intestazione.
 *  - Annotazioni TypeScript rimosse (`: any[]`, `as any`, `: string`).
 *  - L'unico `expect.arrayContaining` è riscritto come `toContain` per
 *    elemento (cercare ADATTAMENTO nel corpo).
 *  - Il percorso della libreria è INVARIATO: `../scripts/lib/…` risolve in
 *    generator/scripts/lib/ qui come in scripts/lib/ sul sito. Idem il drift
 *    guard su `../scripts/create-article.mjs`, che qui legge la copia del
 *    generatore di questo repo (la sola che genera articoli).
 *
 * ────────────────────────────────────────────────────────────────────────────
...
describe('parseItalianNumber', () => {
  ...
  it('returns NaN for non-numbers', () => {
    ...
    expect(parseItalianNumber(null)).toBeNaN();
  });
});
...
describe('tax plausibility — false positives from the corpus audit', () => {
  const silent = (text) => expect(codes(checkTaxPlausibility(text))).toEqual([]);
...
describe('runFactualityGates', () => {
  it('blocks on any critical and reports every issue', () => {
    ...
    // ADATTAMENTO: vitest usava `expect.arrayContaining(...)`; lo shim non ha
    // matcher asimmetrici, quindi la stessa proprieta' e' asserita per elemento.
    for (const code of ['percent-factor-mismatch', 'tax-exceeds-income', 'fabricated-institution']) {
      expect(codes(result.blocking)).toContain(code);
    }
  });
