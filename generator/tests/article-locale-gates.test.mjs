/**
 * PORTATO da valerielinc-ops/frontaliere-si-o-no `tests/article-locale-gates.test.ts`
 * (manifest: `adapted`). Le librerie sotto test — article-locale-lexicon.mjs e
 * il lato locale di article-factuality-gates.mjs — sono arrivate qui col
 * trasporto del generatore, ma la suite era rimasta solo sul sito. Le
 * traduzioni en/de/fr si generano IN questo repo: il gate che ha fermato
 * «frontalieri» tradotto «border guards» va sorvegliato qui.
 *
 * ## ADATTAMENTI rispetto al sito
 *  - `node:test` + generator/tests/lib/expect-shim.mjs al posto di vitest
 *    (questo repo non ha node_modules né npm ci in CI, vedi tests.yml).
 *  - Annotazioni TypeScript rimosse (`: any[]`, `as any`, `(i: any)`).
 *  - Percorsi delle librerie INVARIATI: `../scripts/lib/…` risolve in
 *    generator/scripts/lib/ qui come in scripts/lib/ sul sito.
 *  - Nessun altro cambiamento: fixture e asserzioni sono quelle del sito.
 *
 * ────────────────────────────────────────────────────────────────────────────
...
describe('canonicalNumeric', () => {
  ...
  it('refuses tokens that are not numbers', () => {
    ...
    expect(canonicalNumeric(null)).toBeNaN();
...
describe('checkTranslationNumericConsistency', () => {
  ...
  describe('magnitude twin', () => {
    it('blocks the same digits at a different order of magnitude', () => {
      ...
      const flagged = issues.find((i) => i.code === 'translation-number-magnitude');
  ...
  it('returns nothing when either side is empty', () => {
    ...
    expect(checkTranslationNumericConsistency(null, 'x', 'en')).toEqual([]);
  });
});
...
describe('checkTranslationFalseFriends', () => {
  ...
  it('has nothing to say about Italian, and does not crash on bad input', () => {
    ...
    expect(checkTranslationFalseFriends(null, 'x', 'en')).toEqual([]);
  });
});
...
describe('runFactualityGates on a translation', () => {
  ...
  it('demotes a content claim the Italian original does not make', () => {
    ...
    const flagged = result.issues.find((i) => i.code === 'tax-exceeds-income');
    if (flagged) {
