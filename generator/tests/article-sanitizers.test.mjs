/**
 * PORTATO DAL SITO — `tests/article-sanitizers.test.ts` di
 * valerielinc-ops/frontaliere-si-o-no, dove gira sotto vitest.
 *
 * Perche' qui: `generator/scripts/lib/article-sanitizers.mjs` gira a OGNI
 * generazione su questo repo — e' `create-article.mjs` a chiamarla — e non
 * aveva un solo test da questo lato. `loop-drift-check` non poteva accorgersene:
 * confronta i file del manifest uno per uno e NON vede l'assenza di un test da
 * un lato. Una libreria che gira in produzione senza la sua suite e' invisibile.
 *
 * Le due copie della lib sono byte-identiche (601 righe, stesso sha256), quindi
 * il porting e' quasi meccanico: cambiano le due righe di import qui sotto, e
 * cadono i quattro cast TypeScript `null as unknown as string` (righe 102-103 e
 * 348-349 dell'originale), che qui sarebbero un SyntaxError — `node --test` non
 * fa transpiling. E' l'unica differenza di sorgente: le asserzioni sono intatte.
 * Lo
 * specificatore della lib resta INVARIATO — da `generator/tests/` il path
 * `../scripts/lib/article-sanitizers.mjs` risolve a `generator/scripts/lib/`,
 * che e' dove la lib vive qui.
 *
 * NOTA su cio' che questa suite NON copre, per non ripetere l'equivoco di #75:
 * quella PR ha aggiunto un guard sui soli control character C0 in sei script di
 * scrittura. E' un'altra cosa da questo cluster, ed e' a monte.
 *
 * `sanitizeBodyIt` e' testata qui ma su questo repo non e' importata da nessuno:
 * `create-article.mjs` chiama i tre sanitizer singolarmente, nello stesso ordine.
 * Il test resta perche' pinna il comportamento della lib, non il suo uso.
 */
/**
...
describe('stripCompetitorPromotion', () => {
  ...
  it('handles empty / null / non-string input safely', () => {
    ...
    expect(stripCompetitorPromotion(null).removed).toBe(0);
    expect(stripCompetitorPromotion(undefined).removed).toBe(0);
  });
});
...
`;
  ...
  it('handles empty / null / non-string input safely', () => {
    ...
    expect(stripFabricatedExamples(null).removedSections).toBe(0);
    expect(stripFabricatedExamples(undefined).removedSections).toBe(0);
  });
});
