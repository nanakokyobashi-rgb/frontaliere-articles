 ...
 */
function runBootstrap({ tipSha = TIP_SHA, baseSha = BASE_SHA, servedSha = null } = {}) {
...
const present = url.includes(served);
if (present) {
  fs.writeFileSync(out, 'export const ok = 1;\\n');
...
test('i moduli si scaricano dalla PUNTA di main, non da base.sha', () => {
  ...
  assert.ok(requested.length > 5, `troppe poche richieste registrate: ${requested.length}`);
...
test('un modulo davvero assente nomina il file e il ref, non solo `curl: (22)`', () => {
  // Nessun ref serve il file: e' il caso «modulo rinominato, o dimenticato
  // nella lista di bootstrap». Il messaggio deve far partire la diagnosi dal
  // modulo, non dal job.
  const notFound = runBootstrap({ servedSha: 'deadbeef' });
  ...
  assert.match(notFound.output, /scripts\/ci\/review-gate\.mjs/u,
