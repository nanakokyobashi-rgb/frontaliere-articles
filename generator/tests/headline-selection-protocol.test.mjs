...
test('la selezione headline resta fuori dalla preferenza CLI (decisione del proprietario)', () => {
  const retrySrc = cutDecl('async function requestHeadlineSelection(');
  assert.doesNotMatch(retrySrc, /prefer\s*:/, 'la selezione headline non deve passare `prefer`: vedi il commento su PREFERRED_GENERATION_MODELS');
  assert.match(src, /NON e' passata a: traduzioni, meta, FAQ, classificazione, selezione headline,/);
});
